...
class ItemEnterEventListener(EventListener):
    def on_event(self, event, extension):
        ...
        data = event.get_data()
        logger.debug("uLauncher Keep ItemEnterEventListener, got data: %s" % data)
        on_enter = data["id"]
...
class KeywordQueryEventListener(EventListener):
    def on_event(self, event, extension):
        items = []
        items.append(ExtensionResultItem(icon='images/keep-icon.svg', name="Create", description='Create a new not', on_enter=ExtensionCustomAction({"id: 1"}, keep_app_open=True)))
